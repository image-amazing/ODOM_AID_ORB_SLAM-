...
cv::Mat System::TrackStereoOdom(const cv::Mat &imLeft, const cv::Mat &imRight, const std::vector<OdomData> &vodom, const double &timestamp)
{
    if(mSensor!=STEREO)
    {
    ...
    return mpTracker->GrabImageStereoOdom(imLeft, imRight, vodom, timestamp);
}
...
void System::Shutdown()
{
    ...
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA() || !mpViewer->isFinished())
    {
        usleep(5000);
    }
