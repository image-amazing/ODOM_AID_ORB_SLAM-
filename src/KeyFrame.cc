...
void KeyFrame::UpdateNavStatePRFromTcw(const cv::Mat& Tcw, const cv::Mat& Toc)
{
    unique_lock<mutex> lock(mMutexNavState);
    cv::Mat Two = Converter::toCvMatInverse(Toc*Tcw);
    Matrix3d Rwo = Converter::toMatrix3d(Two.rowRange(0,3).colRange(0,3));
    Vector3d Pwo = Converter::toVector3d(Two.rowRange(0,3).col(3));

    Matrix3d Rw1 = mNavState.Get_RotMatrix();

    mNavState.Set_Pos(Pwo);
    mNavState.Set_Rot(Rwo);
}
...
std::vector<OdomData> KeyFrame::GetVectorOdomData(void)
{
    unique_lock<mutex> lock(mMutexOdomData);
    return mvOdomData;
}

void KeyFrame::AppendOdomDataToFront(KeyFrame* pPrevKF)
{
    std::vector<OdomData> vodomnew = pPrevKF->GetVectorOdomData();
    {
        unique_lock<mutex> lock(mMutexOdomData);
        vodomnew.insert(vodomnew.end(), mvOdomData.begin(), mvOdomData.end());
        mvOdomData = vodomnew;
    }
}

void KeyFrame::UpdatePoseFromNS(const cv::Mat &Toc)
{
    cv::Mat Roc_ = Toc.rowRange(0,3).colRange(0,3).clone();
    cv::Mat Poc_ = Toc.rowRange(0,3).col(3).clone();

    cv::Mat Rwo_ = Converter::toCvMat(mNavState.Get_RotMatrix());
    cv::Mat Pwo_ = Converter::toCvMat(mNavState.Get_P());

    cv::Mat Rcw_ = (Rwo_*Roc_).t();
    cv::Mat Pwc_ = Rwo_*Poc_ + Pwo_;
...
void KeyFrame::UpdateNavState(const OdomPreintegrator& odompreint)
{
    unique_lock<mutex> lock(mMutexNavState);
    Converter::updateNS(mNavState,odompreint);
}
...
void KeyFrame::SetNavStateRot(const SO3Type &rot)
{
    unique_lock<mutex> lock(mMutexNavState);
    mNavState.Set_Rot(rot);
}

const OdomPreintegrator & KeyFrame::GetOdomPreInt(void)
{
    unique_lock<mutex> lock(mMutexOdomData);
    return mOdomPreInt;
}
...
void KeyFrame::ComputePreInt(void)
{
    ...
    else
    {
        ...
        {
            const OdomData& odom = mvOdomData.front();
            double dt = odom._t - mpPrevKeyFrame->mTimeStamp;
            mOdomPreInt.update(odom._v, odom._w, dt);

            // Test log
            if(dt < 0)
            {
                cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this KF vs last imu time: "<<mTimeStamp<<" vs "<<odom._t<<endl;
        ...
        // integrate each imu
        for(size_t i=0; i<mvOdomData.size(); i++)
        {
            const OdomData& odom = mvOdomData[i];
            double nextt;
            if(i==mvOdomData.size()-1)
                nextt = mTimeStamp;         // last IMU, next is this KeyFrame
            else
                nextt = mvOdomData[i+1]._t;  // regular condition, next is imu data
...
KeyFrame::KeyFrame(Frame &F, Map* pMap, KeyFrameDatabase* pKFDB, std::vector<OdomData> vOdomData, KeyFrame* pPrevKF):
...
{   
    ...
    if(pPrevKF && pNextKF)
    {
        ...
        // Update OdomData for NextKF
        pNextKF->AppendOdomDataToFront(this);
