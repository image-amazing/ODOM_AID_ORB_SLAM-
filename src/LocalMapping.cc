...
// 
    ...
    cv::Mat Roc = Toc.rowRange(0,3).colRange(0,3);
    cv::Mat poc = Toc.rowRange(0,3).col(3);
    cv::Mat Rco = Roc.t();
    cv::Mat pco = -Rco*poc;
// 
...
//             cv::Mat wPo = wPc + Rwc*pco;
//             pKF->SetNavStatePos(Converter::toVector3d(wPo));
//             pKF->SetNavStateRot(Converter::toMatrix3d(Rwc*Rco));
// 
//             // Step 4.
//             // compute velocity
...
//                 // Odom pre-int between pKF ~ pKFnext
//                 const OdomPreintegrator& odompreint = pKFnext->GetOdomPreInt();
//                 // Time from this(pKF) to next(pKFnext)
//                 double dt = odompreint.getDeltaTime();                                       // deltaTime
//                 cv::Mat dp = Converter::toCvMat(odompreint.getDeltaP());       		    // deltaP
...
// 
//                 cv::Mat vel = - 1./dt*( (wPc - wPcnext) + (Rwc - Rwcnext)*pco + Rwc*Rco*dp);
	    ...
	    if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>2)
                {
                    ...
                    {
                        Optimizer::LocalBundleAdjustmentNavState(mpCurrentKeyFrame,mlLocalKeyFrames,&mbAbortBA, mpMap, this);
                    }
                }
