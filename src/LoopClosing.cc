...
//     Optimizer::GlobalBundleAdjustemnt(mpMap,10,&mbStopGBA,nLoopKF,false);
    Optimizer::GlobalBundleAdjustmentNavState(mpMap,10,&mbStopGBA,nLoopKF,false);
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            cv::Mat cvToc = ConfigParam::GetMatToc();
			...
			pChild->mNavStateGBA = pChild->GetNavState();
                        cv::Mat TwoGBA = Converter::toCvMatInverse(cvToc*pChild->mTcwGBA);
                        Matrix3d RwoGBA = Converter::toMatrix3d(TwoGBA.rowRange(0,3).colRange(0,3));
                        Vector3d PwoGBA = Converter::toVector3d(TwoGBA.rowRange(0,3).col(3));
                 
                        pChild->mNavStateGBA.Set_Pos(PwoGBA);
                        pChild->mNavStateGBA.Set_Rot(RwoGBA);
                    }
