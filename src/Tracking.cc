...
//-------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------
bool Tracking::TrackLocalMapWithOdom(bool bMapUpdated)
{
    ...
    if(mpLocalMapper->GetFirstVINSInited() || bMapUpdated)
    {
        ...
        OdomPreintegrator odompreint = GetOdomPreIntSinceLastKF(&mCurrentFrame, mpLastKeyFrame, mvOdomSinceLastKF);

        // Test log
        if(mpLocalMapper->GetFirstVINSInited() && !bMapUpdated) cerr<<"1-FirstVinsInit, but not bMapUpdated. shouldn't"<<endl;

        //
        Optimizer::PoseOptimization(&mCurrentFrame,mpLastKeyFrame,odompreint,true);
    }
    // Map not updated, optimize with last Frame
    else
    {
        // Get initial pose from Last Frame
        OdomPreintegrator odompreint = GetOdomPreIntSinceLastFrame(&mCurrentFrame, &mLastFrame);

        Optimizer::PoseOptimization(&mCurrentFrame,&mLastFrame,odompreint,true);
    }
...
void Tracking::PredictNavStateByOdom(bool bMapUpdated)
{
    if(!mpLocalMapper->GetVINSInited()) cerr<<"mpLocalMapper->GetVINSInited() not, shouldn't in PredictNavStateByOdom"<<endl;
    ...
    if(mpLocalMapper->GetFirstVINSInited() || bMapUpdated)
    {
        ...
        // Compute Odom Pre-integration
        mOdomPreIntInTrack = GetOdomPreIntSinceLastKF(&mCurrentFrame, mpLastKeyFrame, mvOdomSinceLastKF);

        // Get initial NavState&pose from Last KeyFrame
        mCurrentFrame.SetInitialNavStateAndBias(mpLastKeyFrame->GetNavState());
        mCurrentFrame.UpdateNavState(mOdomPreIntInTrack);
    ...
    {
        // Compute Odom Pre-integration
        mOdomPreIntInTrack = GetOdomPreIntSinceLastFrame(&mCurrentFrame, &mLastFrame);
...
bool Tracking::TrackWithOdom(bool bMapUpdated)
{
    ORBmatcher matcher(0.9,true);

    // VINS has been inited in this function
    if(!mpLocalMapper->GetVINSInited()) cerr<<"local mapping VINS not inited. why call TrackWithOdom?"<<endl;

    // Predict NavState&Pose by Odom
    // And compute the Odom pre-integration for PoseOptimization
    PredictNavStateByOdom(bMapUpdated);
    ...
    if(mpLocalMapper->GetFirstVINSInited() || bMapUpdated)
    {
        Optimizer::PoseOptimization(&mCurrentFrame,mpLastKeyFrame,mOdomPreIntInTrack,false);
    }
    else
    {
        Optimizer::PoseOptimization(&mCurrentFrame,&mLastFrame,mOdomPreIntInTrack,false);
    }
...
OdomPreintegrator Tracking::GetOdomPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<OdomData>& vOdomSInceLastKF)
{
    // Reset pre-integrator first
    OdomPreintegrator OdomPreInt;
    OdomPreInt.reset();

    // remember to consider the gap between the last KF and the first Odom
    {
        const OdomData& odom = vOdomSInceLastKF.front();
        double dt = odom._t - pLastKF->mTimeStamp;
        OdomPreInt.update(odom._v, odom._w, dt);

        // Test log
        if(dt < 0)
        {
            cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this KF vs last odom time: "<<pLastKF->mTimeStamp<<" vs "<<odom._t<<endl;
            std::cerr.unsetf ( std::ios::showbase );                // deactivate showbase
        }
    }
    // integrate each odom
    for(size_t i=0; i<vOdomSInceLastKF.size(); i++)
    {
        const OdomData& odom = vOdomSInceLastKF[i];
        double nextt;
        if(i==vOdomSInceLastKF.size()-1)
            nextt = pCurF->mTimeStamp;         // last odom, next is this KeyFrame
        else
            nextt = vOdomSInceLastKF[i+1]._t;  // regular condition, next is imu data
...
OdomPreintegrator Tracking::GetOdomPreIntSinceLastFrame(Frame* pCurF, Frame* pLastF)
{
    // Reset pre-integrator first
    OdomPreintegrator OdomPreInt;
    OdomPreInt.reset();

    pCurF->ComputeOdomPreIntSinceLastFrame(pLastF,OdomPreInt);

    return OdomPreInt;
}


cv::Mat Tracking::GrabImageStereoOdom(const cv::Mat &imLeft, const cv::Mat &imRight, const std::vector<OdomData> &vodom, const double &timestamp)
{
    mvOdomSinceLastKF.insert(mvOdomSinceLastKF.end(), vodom.begin(),vodom.end());
//     mImGray = im;
    mImGray = imLeft;
	...
	mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft,mpORBextractorRight, vodom,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    else
	mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft,mpORBextractorRight,vodom,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpLastKeyFrame);
...
//                 bOK = TrackLocalMap();
	    {
                ...
                else
                {
                    ...
                    {
                        bOK = TrackLocalMapWithOdom(bMapUpdated);
                    }
                }
#endif
            }
...
void Tracking::StereoInitialization()
{
	...
	mCurrentFrame.SetNavState(NavState());
	mCurrentFrame.UpdatePoseFromNS(ConfigParam::GetMatToc());
	
        // Create KeyFrame
        KeyFrame* pKFini = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB, mvOdomSinceLastKF, NULL);
	pKFini->SetInitialNavStateAndBias(mCurrentFrame.GetNavState());
	pKFini->ComputePreInt();
	// Clear odom data
        mvOdomSinceLastKF.clear();
...
void Tracking::CreateNewKeyFrame()
{
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB,mvOdomSinceLastKF,mpLastKeyFrame);
