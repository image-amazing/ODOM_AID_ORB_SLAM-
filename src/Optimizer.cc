...
void Optimizer::GlobalBundleAdjustmentNavState(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();

    // Extrinsics
    Matrix4d Toc = ConfigParam::GetEigToc();
    Matrix3d Roc = Toc.topLeftCorner(3,3);
    Vector3d Poc = Toc.topRightCorner(3,1);
...
//     InvCovBgaRW.bottomRightCorner(3,3) = Matrix3d::Identity()/IMUData::getAccBiasRW2();   // Accelerometer bias random walk, covariance INVERSE
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        // PVR edge
        {
            g2o::EdgeNavStatePR * epvr = new g2o::EdgeNavStatePR();
            ...
            epvr->setMeasurement(pKF1->GetOdomPreInt());

            Matrix6d InvCovPVR = pKF1->GetOdomPreInt().getCovPPhi().inverse();
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
       ...
       const map<KeyFrame*,size_t> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            {
// 	      cerr<<"Monocular not supported"<<endl;
            }
            else
            {
//                 cerr<<"Stereo not supported"<<endl;
		Eigen::Matrix<double,3,1> obs;
                ...
                g2o::EdgeNavStatePRStereoPointXYZ* e = new g2o::EdgeNavStatePRStereoPointXYZ();
...
//                 e->bf = pKF->mbf;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        g2o::VertexNavStatePR* vNSPVR = static_cast<g2o::VertexNavStatePR*>(optimizer.vertex(2*pKF->mnId));
        ...
        if(nLoopKF==0)
        {
            ...
            pKF->UpdatePoseFromNS(ConfigParam::GetMatToc());
        }
        else
        {
            ...
            cv::Mat Two_ = cv::Mat::eye(4,4,CV_32F);
            Converter::toCvMat(pKF->mNavStateGBA.Get_RotMatrix()).copyTo(Two_.rowRange(0,3).colRange(0,3));
            Converter::toCvMat(pKF->mNavStateGBA.Get_P()).copyTo(Two_.rowRange(0,3).col(3));
            cv::Mat Twc_ = Two_*ConfigParam::GetMatToc();
...
int Optimizer::PoseOptimization(Frame *pFrame, Frame* pLastFrame, const OdomPreintegrator& odompreint, const bool& bComputeMarg)
{
    // Extrinsics
    Matrix4d Toc = ConfigParam::GetEigToc();
    Matrix3d Roc = Toc.topLeftCorner(3,3);
    ...
    g2o::VertexNavStatePR * vNSFPVRlast = new g2o::VertexNavStatePR();
    {
...
//         eNSPrior->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(LastFrameBiasId)));
    ...
    g2o::EdgeNavStatePR* eNSPVR = new g2o::EdgeNavStatePR();
    {
...
//         eNSPVR->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(LastFrameBiasId)));
    ...
    vector<g2o::EdgeNavStatePRStereoPointXYZOnlyPose*> vpEdgesStereo;
    vector<size_t> vnIndexEdgeStereo;
    vpEdgesStereo.reserve(Ncur);
    vnIndexEdgeStereo.reserve(Ncur);

    vector<g2o::EdgeNavStatePRStereoPointXYZOnlyPose*> vpEdgesStereoLast;
    ...
    for(int i=0; i<Ncur; i++)
    {
        ...
        if(pMP)
        {
            ...
            {
// 		cerr<<"mono shouldn't in poseoptimization"<<endl;
...
//                 cerr<<"stereo shouldn't in poseoptimization"<<endl;
		nInitialCorrespondences++;
                ...
                g2o::EdgeNavStatePRStereoPointXYZOnlyPose* e = new g2o::EdgeNavStatePRStereoPointXYZOnlyPose();
		...
		e->SetParams(pFrame->fx,pFrame->fy,pFrame->cx,pFrame->cy,pFrame->mbf,Roc,Poc,Converter::toVector3d(pMP->GetWorldPos()));
...
//                 cerr<<"stereo shouldn't in poseoptimization"<<endl;
		nInitialCorrespondences++;
                ...
                vpEdgesStereoLast.push_back(e);
                vnIndexEdgeStereoLast.push_back(i);
            }
        }
    }
    }
...
//         vNSFBiaslast->setEstimate(pLastFrame->GetNavState());
        ...
        {
            g2o::EdgeNavStatePRStereoPointXYZOnlyPose* e = vpEdgesStereo[i];
        ...
        {
            g2o::EdgeNavStatePRStereoPointXYZOnlyPose* e = vpEdgesStereoLast[i];
    ...
    g2o::VertexNavStatePR* vNSPVR_recov = static_cast<g2o::VertexNavStatePR*>(optimizer.vertex(FramePVRId));
    const NavState& nsPVR_recov = vNSPVR_recov->estimate();
    NavState ns_recov = nsPVR_recov;
    pFrame->SetNavState(ns_recov);
    pFrame->UpdatePoseFromNS(ConfigParam::GetMatToc());
    ...
    if(bComputeMarg)
    {
        ...
        Matrix<double,6,6> margCov = Matrix<double,6,6>::Zero();
        margCov.topLeftCorner(6,6) = spinv.block(0,0)->eval();
...
int Optimizer::PoseOptimization(Frame *pFrame, KeyFrame* pLastKF, const OdomPreintegrator& odompreint, const bool& bComputeMarg)
{
    // Extrinsics
    Matrix4d Toc = ConfigParam::GetEigToc();
    Matrix3d Roc = Toc.topLeftCorner(3,3);
...
//     const float deltaMono = sqrt(5.991);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(pFrame->mvuRight[i]<0)
            {
// 		cerr<<"monocular shouldn't in poseoptimization"<<endl;
//                 nInitialCorrespondences++;
//                 pFrame->mvbOutlier[i] = false;
// 
//                 Eigen::Matrix<double,2,1> obs;
...
cout << "ni" << endl;
    ...
    if(bComputeMarg)
    {
        ...
        optimizer.computeMarginals(spinv, margVerteces);
        // spinv include 2 blocks, 9x9-(0,0) for PVR, 6x6-(1,1) for Bias
        Matrix<double,6,6> margCovInv = Matrix<double,6,6>::Zero();
...
void Optimizer::LocalBundleAdjustmentNavState(KeyFrame *pCurKF, const std::list<KeyFrame*> &lLocalKeyFrames, bool* pbStopFlag, Map* pMap, LocalMapping* pLM)
{
    ...
    for(list<KeyFrame*>::const_iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        // Vertex of PVR
        {
            g2o::VertexNavStatePR * vNSPVR = new g2o::VertexNavStatePR();
    ...
    vector<g2o::EdgeNavStatePR*> vpEdgesNavStatePVR;
    ...
    //const float thHuberNavState = sqrt(30.5779);
    const float thHuberNavStatePVR = sqrt(16.812);
    ...
    for(list<KeyFrame*>::const_iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        // Test log
        if(pKF1->GetOdomPreInt().getDeltaTime() < 1e-3)
        {
            cerr<<"Odom pre-integrator delta time between 2 KFs too small: "<<pKF1->GetOdomPreInt().getDeltaTime()<<endl;
    ...
    vector<g2o::EdgeNavStatePRStereoPointXYZ*> vpEdgesStereo;
...
//     const float thHuberMono = sqrt(5.991);
    const float thHuberStereo = sqrt(7.8147);
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
		...
		const float &kp_ur = pKFi->mvuRight[mit->second];
                ...
                {
// 		    cerr<<"Monocular not supported yet, why here?? check."<<endl;
//                     Eigen::Matrix<double,2,1> obs;
//                     obs << kpUn.pt.x, kpUn.pt.y;
// 
...
//                     cerr<<"Stereo not supported yet, why here?? check."<<endl;
		    ...
		    e->SetParams(pKFi->fx,pKFi->fy,pKFi->cx,pKFi->cy,pKFi->mbf,Roc,Poc);
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
    {
        g2o::EdgeNavStatePRStereoPointXYZ* e = vpEdgesStereo[i];
        MapPoint* pMP = vpMapPointEdgeStereo[i];

        if(pMP->isBad())
            continue;

        if(e->chi2()>7.8147 || !e->isDepthPositive())
        {
            e->setLevel(1);
        }
...
//    //cout<<"pose-point chi2: "<<PosePointchi2<<", pose-pose chi2: "<<PosePosechi2<<endl;
    ...
    {
        KeyFrame* pKFi = *lit;
        g2o::VertexNavStatePR* vNSPVR = static_cast<g2o::VertexNavStatePR*>(optimizer.vertex(2*pKFi->mnId));
        ...
        // Update pose Tcw
        pKFi->UpdatePoseFromNS(ConfigParam::GetMatToc());
				       ...
				       LoopClosing* pLC)
{
    ...
    cv::Mat Toc = ConfigParam::GetMatToc();
	...
	// Update P/V/R in NavState
        pKFi->UpdateNavStatePRFromTcw(Tiw,Toc);
    }
