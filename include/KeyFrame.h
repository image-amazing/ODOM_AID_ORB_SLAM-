...
public:
    ...
    KeyFrame(Frame &F, Map* pMap, KeyFrameDatabase* pKFDB, std::vector<OdomData> vOdomData, KeyFrame* pLastKF=NULL);
    ...
    std::vector<OdomData> GetVectorOdomData(void);
    void AppendOdomDataToFront(KeyFrame* pPrevKF);
    void ComputePreInt(void);

    const OdomPreintegrator & GetOdomPreInt(void);

    void UpdateNavStatePRFromTcw(const cv::Mat &Tcw,const cv::Mat &Toc);
    void UpdatePoseFromNS(const cv::Mat &Toc);
    void UpdateNavState(const OdomPreintegrator& odompreint);
    ...
    void SetNavStateRot(const SO3Type &rot);
...
protected:
    ...
    // Odom Data from lask KeyFrame to this KeyFrame
    std::mutex mMutexOdomData;
    std::vector<OdomData> mvOdomData;
